...
header-includes:
  ...
  - |
    ...
    \usepackage{lastpage}
  - |
    \fancyhead[L]{Biuro Informatyki i Łączności - KG PSP} \fancyhead[R]{Strona \thepage\ z \pageref{LastPage}}
  - |
    \fancyfoot[R]{\vspace{0.5cm}  }
  - |
    \renewcommand{\headrulewidth}{0.5pt}
  - |
    \renewcommand{\footrulewidth}{0.3pt}
  - |
    \renewcommand{\footrule}{\vspace{0.05cm}\hrule width \hsize height \footrulewidth \vspace{0.3cm}}
  - |
...
\fill[red!80!black] (current page.north west) rectangle ([yshift=-3cm]current page.north east);
\fill[red!60!black] ([yshift=-3cm]current page.north west) rectangle ([yshift=-3.3cm]current page.north east);
\node[anchor=north, text=white, font=\Large\bfseries] at ([yshift=-0.7cm]current page.north) {KOMENDA GŁÓWNA PAŃSTWOWEJ STRAŻY POŻARNEJ};
\node[anchor=north, text=white, font=\Large] at ([yshift=-1.7cm]current page.north) {BIURO INFORMATYKI I ŁĄCZNOŚCI};
\end{tikzpicture}

\vspace*{1cm}
\vspace{0.5cm}
\begin{center}
\includegraphics[width=4cm]{logo_psp_200.png}
\end{center}
\vspace{0.5cm}

\begin{center}
{\LARGE \textbf{PROJEKT PoC}}
...
\vspace{0.3cm}
{\Large \textbf{O uruchomienie prototypu cyfrowego\\
„nieśmiertelnika" dla strażaków\\
PSP i OSP}}

\vspace{0.3cm}
{\normalsize \textit{Projekt dokumentacji technicznej i uzasadnienie}}

\vspace{1cm}
\begin{tikzpicture}
\node[draw=red!60!black, line width=1pt, rounded corners, inner sep=15pt, minimum width=10cm] {
  \begin{minipage}{9cm}
    \centering
    \textbf{Opracował:}\\
...
\begin{tikzpicture}[remember picture, overlay]
\fill[red!80!black] (current page.south west) rectangle ([yshift=2cm]current page.south east);
\fill[red!60!black] ([yshift=2cm]current page.south west) rectangle ([yshift=2.3cm]current page.south east);

\node[anchor=south, text=white, font=\small] at ([yshift=0.5cm]current page.south) {Wersja 3.3 | Warszawa, sierpień 2025 r.};
\end{tikzpicture}
\end{center}
