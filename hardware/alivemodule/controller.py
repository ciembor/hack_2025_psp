...
class Controller:
    ...
    def __init__(self,
        ...
        self.pin_led = self.prepare_pin(pin_id_led) if pin_id_led is not None else None
        self.on_board_led_high_is_on = on_board_led_high_is_on
        self.pin_reset = self.prepare_pin(pin_id_reset) if pin_id_reset is not None else None
    ...
    def led_on(self, on=True):
        if self.pin_led is None:
            return
        if self.on_board_led_high_is_on == on:
            self.pin_led.high()
        else:
            self.pin_led.low()
